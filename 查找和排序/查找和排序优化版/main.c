...
void quickSort(int arr[], int n) { quickSortRec(arr, 0, n-1); }

// ---------- 优化的测试函数 ----------
double testSort(void (*sortFunc)(int[], int), int original[], int n, int timeoutSec) {
    // 优化：避免 O(n²) 算法在超大输入下实际运行
    if (n >= 50000) {
        if (sortFunc == bubbleSort || sortFunc == selectionSort)
            return -1.0;          // 直接标记超时
        if (sortFunc == insertionSort && n >= 100000)
            return -1.0;          // 大随机数据下插入排序也超时
    }
    ...
    if (elapsed > timeoutSec) return -1.0;
    return elapsed;
}
...
void printTable(int sizes[], int numSizes, const char* dataType,
    ...
    for (int i = 0; i < numSizes; i++) {
        printf("%-10d", sizes[i]);
        for (int j = 0; j < 5; j++) {
            if (timeoutFlag[i][j]) printf("%-14s", ">300s");
            else if (results[i][j] < 0) printf("%-14s", "错误");
            else printf("%-14.4f", results[i][j]);
        }
        printf("\n");
    }
}
