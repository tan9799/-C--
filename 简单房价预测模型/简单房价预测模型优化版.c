...
int main() {
    ...
    // 输出相关系数
    printf("\n各特征与房价的相关系数：\n");
    const char* feat_names[] = {"CRIM","ZN","INDUS","CHAS","NOX","RM","AGE","DIS","RAD","TAX","PTRATIO","B","LSTAT"};
    for (j = 0; j < FEATURES; j++) {
        printf("特征 %2d (%6s) : %.4f\n", j + 1, feat_names[j], entries[j].corr);
    }

    // 找出最相关特征
    qsort(entries, FEATURES, sizeof(CorrEntry), cmp_corr);
    int best_idx = entries[0].idx;
    printf("\n最相关特征: 特征 %d (%s) 相关系数 = %.4f\n", best_idx + 1, feat_names[best_idx], entries[0].corr);

    // ================== (1) 一元线性回归（固定特征6: RM） ==================
    int demo_feat = 5;  // 索引5对应RM
    ...
    printf("\n=== (2) 基于最相关特征的一元线性回归（特征%d: %s）===\n", best_idx + 1, feat_names[best_idx]);
    ...
    printf("截距 = %.4f\n", theta_all[0]);
    for (j = 0; j < FEATURES; j++) {
        printf("系数 %2d (%6s) = %.4f\n", j+1, feat_names[j], theta_all[j+1]);
    }
    printf("测试集 RMSE = %.4f\n", rmse_all);
    ...
    printf("所选特征索引: ");
    for (j = 0; j < m; j++) printf("%d (%s) ", top_indices[j]+1, feat_names[top_indices[j]]);
    printf("\n模型系数:\n");
    printf("截距 = %.4f\n", theta_top[0]);
    for (j = 0; j < m; j++) {
        printf("系数 %2d (%6s) = %.4f\n", j+1, feat_names[top_indices[j]], theta_top[j+1]);
    }
    printf("测试集 RMSE = %.4f\n", rmse_top);
